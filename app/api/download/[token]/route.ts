...
export async function GET(_req: NextRequest, { params }: { params: { token: string } }) {
  try {
    const { data: p, error } = await db().from('purchases').select('*, product:products(zip_file_url)').eq('download_token', params.token).single();
    if (error || !p) return NextResponse.json({ error: 'Invalid link' }, { status: 404 });
